...
public class ActivityWCTianLaiCarCD extends BaseActivity {
    ...
    public void updaterDiscState() {
        int havedisc = DataCanbus.DATA[95] & 65535;
        int disc = (DataCanbus.DATA[97] >> 8) & 255;
        int track = DataCanbus.DATA[97] & 255;
        if (((havedisc >> 0) & 1) == 1) {
            sHasDisc |= 1;
        } else {
            sHasDisc &= 254;
    ...
    public void updaterPlayTime() {
        int playminute = (DataCanbus.DATA[98] >> 8) & 255;
        int playsecond = DataCanbus.DATA[98] & 255;
        if (playminute > 59) {
            playminute = 59;
        }
        if (playminute < 0) {
    ...
    public void updaterMode() {
        int mode = DataCanbus.DATA[100] & 255;
