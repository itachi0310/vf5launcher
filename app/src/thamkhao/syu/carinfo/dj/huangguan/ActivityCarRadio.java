...
public class ActivityCarRadio extends Activity implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (event.getAction() == 0) {
            switch (v.getId()) {
                case R.id.btn_am_radio0 /* 2131430068 */:
                    this.curnum = 1;
                    mUpdatermCalTime(true);
                    break;
                case R.id.btn_am_radio1 /* 2131430069 */:
                    this.curnum = 2;
                    mUpdatermCalTime(true);
                    break;
                case R.id.btn_am_radio2 /* 2131430070 */:
                    this.curnum = 3;
                    mUpdatermCalTime(true);
                    break;
                case R.id.btn_am_radio3 /* 2131430071 */:
                    this.curnum = 4;
                    mUpdatermCalTime(true);
                    break;
                case R.id.btn_am_radio4 /* 2131430072 */:
                    this.curnum = 5;
                    mUpdatermCalTime(true);
                    break;
                case R.id.btn_am_radio5 /* 2131430073 */:
                    this.curnum = 6;
                    mUpdatermCalTime(true);
                    break;
            }
    ...
    protected void onResume() {
        ...
        if (DataCanbus.DATA[1000] != 590253 && DataCanbus.DATA[1000] != 655789 && DataCanbus.DATA[1000] != 721325 && DataCanbus.DATA[1000] != 786861 && DataCanbus.DATA[1000] != 852397 && DataCanbus.DATA[1000] != 917933 && DataCanbus.DATA[1000] != 983469 && DataCanbus.DATA[94] != 1) {
            DataCanbus.PROXY.cmd(2, 5, 1);
            DataCanbus.PROXY.cmd(2, 5, 0);
        }
        mIsFront = true;
    ...
    public void mUpdaterRadioCurrentChannelPreset() {
        ...
        if (value >= 1 && value <= 6) {
            Current_preset_station = DataCanbus.DATA[97];
        }
