...
public class TianLai09Act extends BaseActivity {
    ...
    public void updaterShowPage() {
        int PageIndex = DataCanbus.DATA[112] & 3;
        if (PageIndex <= 3) {
            if (PageIndex == 1) {
    ...
    public void updaterVol() {
        ...
        int enable = (DataCanbus.DATA[111] >> 8) & 255;
        if (enable == 1) {
    ...
    public void updaterDiscState() {
        int havedisc = DataCanbus.DATA[102] & 65535;
        int disc = (DataCanbus.DATA[103] >> 8) & 255;
        int track = DataCanbus.DATA[103] & 255;
        if (((havedisc >> 14) & 1) == 1) {
            sHasDisc |= 1;
        } else {
            sHasDisc &= 254;
    ...
    public void updaterPlayTime() {
        int playminute = (DataCanbus.DATA[104] >> 8) & 255;
        if (playminute > 59) {
            playminute = 59;
        }
        if (playminute < 0) {
        ...
        int playsecond = DataCanbus.DATA[104] & 255;
        if (playsecond > 59) {
            playsecond = 59;
        }
        if (playsecond < 0) {
    ...
    public void updaterMediaState() {
        int media_type = (DataCanbus.DATA[109] >> 5) & 255;
        int media_data = DataCanbus.DATA[109] & 31;
        if (media_type != 0) {
            updatesound(1);
        } else {
            updaterShowPage();
