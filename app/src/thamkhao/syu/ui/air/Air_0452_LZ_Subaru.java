...
public class Air_0452_LZ_Subaru extends AirBase {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (TempUnit == 1) {
            this.srPath.addRect(129.0f, 118.0f, 166.0f, 158.0f, Path.Direction.CW);
            this.srPath.addRect(974.0f, 114.0f, 1014.0f, 158.0f, Path.Direction.CW);
        }
        ...
        int temp = this.DATA[27] + (this.DATA[135] * 2);
        if (temp == -1) {
            ...
            c.drawText(new StringBuilder().append((((temp + 8) * 9) + 320) / 10).toString(), 70.0f, 135.0f, this.mPaint);
        } else {
            c.drawText(new StringBuilder().append(temp / 2.0f).toString(), 70.0f, 135.0f, this.mPaint);
        }
        int temp2 = this.DATA[28] + (this.DATA[136] * 2);
        if (temp2 == -1) {
            ...
            c.drawText(new StringBuilder().append((((temp2 + 8) * 9) + 320) / 10).toString(), 915.0f, 135.0f, this.mPaint);
        } else {
            c.drawText(new StringBuilder().append(temp2 / 2.0f).toString(), 915.0f, 135.0f, this.mPaint);
        }
        canvas.save();
