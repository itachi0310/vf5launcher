...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i2 = 0; i2 < this.mDragOutlineAnims.length; i2++) {
            ...
            anim.getAnimator().addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.launcher17.CellLayout.1
            ...
            anim.getAnimator().addListener(new AnimatorListenerAdapter() { // from class: com.android.launcher17.CellLayout.2
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFixedCellWidth < 0 || this.mFixedCellHeight < 0) {
            ...
            if (cw != this.mCellWidth || ch != this.mCellHeight) {
                ...
                if (this.mIsHotseat) {
                    int iMax = Math.max(cw, ch);
                    this.mCellHeight = iMax;
                    this.mCellWidth = iMax;
                } else {
                    this.mCellWidth = cw;
                    this.mCellHeight = ch;
                }
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration, int delay, boolean permanent, boolean adjustOccupied) {
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            va.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.launcher17.CellLayout.3
                @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                public void onAnimationUpdate(ValueAnimator animation) {
            ...
            va.addListener(new AnimatorListenerAdapter() { // from class: com.android.launcher17.CellLayout.4
    ...
    /* JADX WARN: Code duplicated, block: B:106:0x0234  */
    /* JADX WARN: Code duplicated, block: B:129:0x0242 A[SYNTHETIC] */
    /* JADX WARN: Code duplicated, block: B:131:? A[LOOP:7: B:46:0x0145->B:131:?, LOOP_END, SYNTHETIC] */
                    ...
                    if (ignoreOccupied) {
                        ...
                        while (true) {
                            if (i < minSpanX) {
                                ...
                                currentRect = this.mTempRectStack.pop();
                                ...
                                distance = Math.sqrt(Math.pow(cellXY[0] - pixelX2, 2.0d) + Math.pow(cellXY[1] - pixelY2, 2.0d));
                        ...
                        validRegions.push(currentRect);
                        distance = Math.sqrt(Math.pow(cellXY2[0] - pixelX2, 2.0d) + Math.pow(cellXY2[1] - pixelY2, 2.0d));
    ...
    private boolean pushViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop, int[] direction, View dragView, ItemConfiguration currentState) {
        ...
        Iterator<View> it2 = cluster.views.iterator();
        while (it2.hasNext()) {
            CellAndSpan c3 = currentState.map.get(it2.next());
    ...
    class ReorderHintAnimation {
        ...
        public ReorderHintAnimation(View child, int cellX0, int cellY0, int cellX1, int cellY1, int spanX, int spanY) {
            ...
            if (dX != dY || dX != 0) {
                if (dY == 0) {
                    ...
                    this.finalDeltaX = (int) (((double) (-Math.signum(dX))) * Math.abs(Math.cos(angle) * ((double) CellLayout.this.mReorderHintAnimationMagnitude)));
                    this.finalDeltaY = (int) (((double) (-Math.signum(dY))) * Math.abs(Math.sin(angle) * ((double) CellLayout.this.mReorderHintAnimationMagnitude)));
                }
            }
        ...
        void animate() {
            ...
            if (this.finalDeltaX != 0.0f || this.finalDeltaY != 0.0f) {
                ...
                va.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.android.launcher17.CellLayout.ReorderHintAnimation.1
                    @Override // android.animation.ValueAnimator.AnimatorUpdateListener
                    public void onAnimationUpdate(ValueAnimator animation) {
                        float r = ((Float) animation.getAnimatedValue()).floatValue();
                ...
                va.addListener(new AnimatorListenerAdapter() { // from class: com.android.launcher17.CellLayout.ReorderHintAnimation.2
                    @Override // android.animation.AnimatorListenerAdapter, android.animation.Animator.AnimatorListener
