...
public class CropView extends TiledImageView implements ScaleGestureDetector.OnScaleGestureListener {
    ...
    /* JADX WARN: Code duplicated, block: B:7:0x000d A[Catch: all -> 0x0031, TryCatch #0 {, blocks: (B:5:0x0005, B:7:0x000d, B:8:0x002f), top: B:13:0x0005 }] */
    private void updateMinScale(int w, int h, TiledImageRenderer.TileSource source, boolean resetScale) {
        synchronized (this.mLock) {
            if (resetScale) {
                ...
                float imageWidth2 = imageDims2[0];
                float imageHeight2 = imageDims2[1];
                this.mMinScale = Math.max(w / imageWidth2, h / imageHeight2);
                this.mRenderer.scale = Math.max(this.mMinScale, this.mRenderer.scale);
            }
            throw th;
        }
    ...
    public void moveToLeft() {
        if (getWidth() == 0 || getHeight() == 0) {
            ...
            observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() { // from class: com.android.launcher17.CropView.1
        ...
        getEdgesHelper(edges);
        float scale = this.mRenderer.scale;
        this.mCenterX = (float) (((double) this.mCenterX) + Math.ceil(edges.left / scale));
        updateCenter();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == 0) {
            ...
            if (this.mTouchCallback != null) {
                if (squaredDist < slop && now < this.mTouchDownTime + ((long) ViewConfiguration.getTapTimeout())) {
