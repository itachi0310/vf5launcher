...
public class DeleteHandler extends DataHandler {
    ...
    private void deleteGenericData(Class<?> modelClass, List<Field> supportedGenericFields, long... ids) {
        for (Field field : supportedGenericFields) {
            ...
            int loopCount = (length - 1) / HttpStatus.SC_INTERNAL_SERVER_ERROR;
            for (int i = 0; i <= loopCount; i++) {
                StringBuilder whereClause = new StringBuilder();
                boolean needOr = false;
                for (int j = HttpStatus.SC_INTERNAL_SERVER_ERROR * i; j < (i + 1) * HttpStatus.SC_INTERNAL_SERVER_ERROR && j < length; j++) {
                    long id = ids[j];
                    if (needOr) {
