...
public final class Utilities {
    ...
    public static Bitmap createIconBitmap_minify(Drawable icon) {
        ...
        synchronized (sCanvas) {
            ...
            int textureWidth = sIconTextureWidth;
            int textureHeight = sIconTextureHeight;
            bitmap2 = Bitmap.createBitmap(textureWidth, textureHeight, Bitmap.Config.ARGB_8888);
            Canvas canvas = sCanvas;
            canvas.setBitmap(bitmap2);
            int i = (textureWidth - width) / 2;
            int i2 = (textureHeight - height) / 2;
            double scale_w = ((double) width) / ((double) sIconSize);
            double scale_h = ((double) height) / ((double) sIconSize);
            int width2 = (int) (((double) width) / scale_w);
            int height2 = (int) (((double) height) / scale_h);
    ...
    static Bitmap createIconBitmap_enlarge1(Drawable icon, Context context) {
        Bitmap bitmap;
        synchronized (sCanvas) {
            if (sIconWidth == -1) {
                initStatics(context);
            ...
            int left = (textureWidth - width) / 2;
            int top = (textureHeight - height) / 2;
            Drawable micon = context.getResources().getDrawable(R.drawable.icon_bg);
            micon.getIntrinsicWidth();
            micon.getIntrinsicHeight();
            micon.draw(canvas);
            micon.setBounds(sOldBounds);
    ...
    static Bitmap createIconBitmap_minify1(Drawable icon, Context context) {
        Bitmap bitmap;
        synchronized (sCanvas) {
            if (sIconWidth == -1) {
                initStatics(context);
            ...
            int height2 = (int) (((double) height) / scale_h);
            int left = (textureWidth - width2) / 2;
            int top = (textureHeight - height2) / 2;
            Drawable micon = context.getResources().getDrawable(R.drawable.icon_bg + sIconCnt);
            if (context.getResources().getBoolean(R.bool.apps_thrid_icon)) {
                sIconCnt++;
                if (sIconCnt >= context.getResources().getInteger(R.integer.utility_minfiy_icon_count)) {
                    sIconCnt = 0;
                }
            }
            int mwidth = micon.getIntrinsicWidth();
            ...
            micon.setBounds(sOldBounds);
            int gap = LauncherApplication.sApp.getResources().getInteger(R.integer.utility_minfiy_gap);
            int offest = Utils.getNameToInteger("utility_minfiy_top_offest");
            int LeftOffest = Utils.getNameToInteger("utility_minfiy_left_offest");
            sOldBounds.set(icon.getBounds());
            icon.setBounds((left + gap) - LeftOffest, (top + gap) - offest, ((left + width2) - gap) - LeftOffest, ((top + height2) - gap) - offest);
            icon.draw(canvas);
            icon.setBounds(sOldBounds);
