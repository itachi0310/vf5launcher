...
public class WeightWatcher extends LinearLayout {
    ...
    public WeightWatcher(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mHandler = new Handler() { // from class: com.android.launcher17.WeightWatcher.1
        ...
        ServiceConnection connection = new ServiceConnection() { // from class: com.android.launcher17.WeightWatcher.2
    ...
    public class ProcessWatcher extends LinearLayout {
        ...
        public void update() {
            ...
            StringBuilder sbAppend = new StringBuilder("(").append(this.mPid);
            if (this.mPid == Process.myPid()) {
                str = "/A";
            } else {
                str = "/S";
            }
            textView.setText(sbAppend.append(str).append(") up ").append(getUptimeString()).append(" P=").append(this.mMemInfo.currentPss).append(" U=").append(this.mMemInfo.currentUss).toString());
        ...
        public class GraphView extends View {
            ...
            public void onDraw(Canvas c) {
                ...
                if (ProcessWatcher.this.mMemInfo != null) {
                    ...
                    float scale = h / ProcessWatcher.this.mMemInfo.max;
                    for (int i = 0; i < N; i++) {
                        float x = i * barStep;
                        c.drawRect(x, h - (ProcessWatcher.this.mMemInfo.pss[i] * scale), x + barWidth, h, this.pssPaint);
                        c.drawRect(x, h - (ProcessWatcher.this.mMemInfo.uss[i] * scale), x + barWidth, h, this.ussPaint);
                    }
