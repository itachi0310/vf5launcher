...
public class IconCache {
    ...
    public void getTitleAndIcon(AppInfo application, ResolveInfo info, HashMap<Object, CharSequence> labelCache) {
        synchronized (this.mCache) {
            ...
            application.iconBitmap1 = setIcon1(application.getPackageName(), entry, info);
            application.appIcon = entry.icon;
        }
    }
    ...
    public Bitmap getIcon(Intent intent) {
        ...
        synchronized (this.mCache) {
            ...
            if (resolveInfo == null || component == null) {
                ...
                icon = setIcon(component.getPackageName(), entry, resolveInfo);
            }
        }
        return icon;
    }
    ...
    public Bitmap getIcon(ComponentName component, ResolveInfo resolveInfo, HashMap<Object, CharSequence> labelCache, ShortcutInfo info) {
        Bitmap icon;
        synchronized (this.mCache) {
            try {
                if (resolveInfo == null || component == null) {
    ...
    private Bitmap setIcon1(String packageName, CacheEntry entry, ResolveInfo info) {
        Resources res = this.mContext.getResources();
        int resid = CustomIcons.getIcon(info.activityInfo);
        return resid != 0 ? Utilities.createIconBitmap_enlarge1(res.getDrawable(resid), this.mContext) : Utilities.createIconBitmap_minify1(new BitmapDrawable(entry.icon), this.mContext);
    }

    public CacheEntry cacheLocked(ComponentName componentName, ResolveInfo info, HashMap<Object, CharSequence> labelCache) {
